...
@section('content')
        <div class="cointainer" style="margin-left: 10%; margin-right: 10%; margin-top: 2%">
            <form class="row g-3" action="{{route("cadastro")}}" method="POST">
                @csrf
                <div>
                    <label for="validationServer01" class="form-label">Nome Completo</label>
                    <input type="text" class="form-control" id="nomeprestador" name="nomeprestador" required>
                </div>
                <div>
                    <label for="exampleFormControlInput1" class="form-label">Endereço de Email</label>
                    <input type="email" class="form-control" id="emailprestador" name="emailprestador" placeholder="seuemail@exemplo.com" required>
                </div>
                <div>
                    <label class="form-label">Telefone para contato</label>
                    <input type="text" class="form-control" id="numero" name="telefoneprestador" placeholder="(99)99999-9999" required>
                </div>
                <div>
                    <select class="form-select" aria-label="Default select example" name="area">
                        <option selected>Selecione sua principal área de atuação</option>
                        @foreach($areas as $area)
                        <option value="{{$area->areaprincipal}}">{{$area->areaprincipal}}</option>
                        @endforeach
                    </select>
                      <label class="form-label">*Escolha apenas a sua PRINCIPAL área, depois do seu registro será possível que você adicione outras áreas ao seu perfil.
                </div>
                <div class="mb-3">
